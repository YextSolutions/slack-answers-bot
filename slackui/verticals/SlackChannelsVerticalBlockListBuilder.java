...
public class SlackChannelsVerticalBlockListBuilder implements VerticalBlockListBuilder {
  private static final String BUTTON_TEXT = ":slack:  View Channel";
  private static final String CHANNEL_MEMBER_COUNT_FIELD_TEXT = "Number of Members";
  ...
  @Inject
  SlackChannelsVerticalBlockListBuilder(
  ...
  private List<SlackBlock> buildVerticalBlockList(AnswersResultData data) {
    try {
      ...
      if (data.name().isEmpty() || data.slackChannelUrl().isEmpty()) {
        throw new PermanentErrorException();
      }
      blocks.add(
          textBuilder.buildLinkMarkdownSection(data.slackChannelUrl().get(), data.name().get()));
      data.slackChannelMemberCount()
          .map(
              count ->
                  textBuilder.buildFieldMarkdownSection(CHANNEL_MEMBER_COUNT_FIELD_TEXT, count))
          .ifPresent(blocks::add);
      data.slackChannelDescription().map(textBuilder::buildPlainTextSection).ifPresent(blocks::add);
